...
**给宿主一个隐藏控制台让子进程继承**（`AllocConsole` 加隐藏）。否决：GUI 宿主调用 AllocConsole 在 Windows 上会触发 Windows Terminal 或控制台弹出一个无内容的空窗口，且若对子进程省略 windowsHide 会导致子进程弹出无内容的 PowerShell 控制台窗口。
