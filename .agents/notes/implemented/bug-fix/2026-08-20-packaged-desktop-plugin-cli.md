...
Plugin Market installs and `dsh plugin --profile web add` against the packaged exe run pnpm in `.config/profiles/<name>` while the GUI stays the owner of the single-instance pipe. Relative plugin and `--patch` specs resolve against the caller's cwd. `--profile`, help, version, and config dumps also reach the CLI. Double-click and a second GUI start are unchanged, and the packaged desktop no longer opens a second default-browser helper. Tests in `apps/cli/tests/packaged-web-entry.spec.ts` pin CLI heads versus `--port` and that CLI heads do not chdir; `apps/cli/tests/desktop-listen.spec.ts` pins `--no-open`; `packages/bundle/web-app/tests/web-app.spec.ts` pins the OS opener for a product exe.
