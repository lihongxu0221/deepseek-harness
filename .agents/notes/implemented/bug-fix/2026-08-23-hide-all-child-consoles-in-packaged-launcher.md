...
**Give the host a hidden console so children inherit it** (`AllocConsole` plus hide). Rejected: calling AllocConsole in a GUI host on Windows allocates a console window or launches Windows Terminal with an empty PowerShell tab, and omitting windowsHide on children causes visible empty PowerShell console popups.
