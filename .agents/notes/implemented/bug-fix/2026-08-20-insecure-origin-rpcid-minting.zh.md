...
`AbstractApiClient.mintRpcId` 通过包内 `randomUuid()` 签发：用 `crypto.getRandomValues` 生成 RFC 4122 UUID v4，并在进程内写入版本与变体位。每一个 `IApiClient` 一元调用——包括 `agentPreset.list` 与 `llm.providers`——都经过该方法。通用 Connection RPC 路径继续使用既有的 `packages/client/connection/src/client/random-uuid.ts`。
...
`packages/host/apiproxy/tests/fetch-carrier.spec.ts` 把 `crypto` 桩成仅有 `getRandomValues`，并断言 `sessions.list` 仍签发 version-4 id。`packages/client/connection/tests/client-apply.client.spec.ts` 在 LAN IP 源下挂载真实 `WebApiClient`、使用同一桩，并往返 `agentPreset.list` 与 `llm.providers`。
