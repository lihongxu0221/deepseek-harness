...
**Reading configuration is as privileged as writing it.** `settings.describe` and `credentials.describe` ride the outer trusted-host fence with the rest of the configuration plane; native desktop openers stay loopback-only ([trusted LAN configuration](2026-08-20-trusted-lan-configuration-plane.md)). The model catalog (`llm.providers`, `llm.models`) never needed the extra pin: it carries provider ids, display names, and model lists — no endpoints, no key state — and a LAN client's model picker needs it. The boundary is asserted over a real HTTP server rather than a hand-assembled request, because the `Host` header a browser actually sends is what decides it.
