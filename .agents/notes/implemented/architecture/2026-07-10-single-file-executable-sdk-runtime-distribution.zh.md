...
配置发现优先使用 `DSH_CORDIS_CONFIG`，其次使用 argv 位置参数；已指定但文件缺失时立即报错。未打包启动和 node 载体没有回退。打包后的可执行文件在两个通道都为空时使用 `<executable-dir>/cordis.yml`，文件不存在就写入捆绑的默认配置，因此实际启动的插件仍来自外部文件（[同目录默认配置](../feature/2026-08-17-packaged-exe-sidecar-default-cordis-yml.md)）。
