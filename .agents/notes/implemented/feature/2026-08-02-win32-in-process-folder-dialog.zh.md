...
- 打包二进制路径会把 `lib/worker.cjs` spawn 给 `process.execPath`，除非 `$NODE_BINARY` 或 `$npm_node_execpath` 指向真正的 node。不是 `node` 的 SEA 宿主必须导入该脚本，而不是启动其产品入口。自动化测试覆盖源码侧、普通 node 下的构建 worker 以及可执行文件解析；打包自我 spawn 仍推迟到 Windows CI 路线图。
