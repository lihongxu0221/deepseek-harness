...
- [`dsh-host-desktop-update`](../../../../packages/host/desktop-update/README.md) reads `VERSION` beside the exe, lists GitHub Releases (prereleases included; `/releases/latest` skips them), downloads the configured `dsh-web-win-x64-` zip, verifies the GitHub `sha256` digest, extracts, and arms a PowerShell helper through `wscript.exe`.
...
Control routes live at `/api/desktop-update/*` on Connection's authenticated Fetch channel. The extra fence allows loopback Hostnames and every current non-internal IPv4 address of this machine (a host may have several) and refuses any other Host, so a request whose Host is not this machine cannot start a 270MB download or replace the product folder. The `/api` HTTP bridge synthesizes Fetch URLs on `http://dsh.internal`, so the extra fence reads the Host header and falls back to the request URL hostname only when Host is absent. Apply never copies `.config`. The helper waits for this PID to exit, shows a WinForms splash, robocopies the extract tree with `/XD .config`, and relaunches `dsh-web.exe`. Node then `process.exit(0)` after one second; the Windows tray host already quits when stdin closes. The helper spawn is owned by [the apply-helper note](../bug-fix/2026-09-06-desktop-update-apply-helper-detached-powershell.md).
