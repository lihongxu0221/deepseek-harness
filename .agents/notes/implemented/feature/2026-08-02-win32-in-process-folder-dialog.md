...
- The packaged-binary arm spawn()s `lib/worker.cjs` under `process.execPath` unless `$NODE_BINARY` or `$npm_node_execpath` names a real node. A SEA host that is not `node` must import that script instead of booting its product entry. Automated tests cover the source plane, the built worker under plain node, and the executable resolver; the packaged self-spawn remains deferred to the Windows CI roadmap.
