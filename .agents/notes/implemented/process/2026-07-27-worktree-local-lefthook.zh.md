...
安装或移除任一 worktree 不再改变其他 worktree 的生效钩子、二进制文件路径或生成的钩子字节。并发安装会串行执行，重复安装保持幂等；[快速本地 Git 钩子](2026-07-22-fast-local-git-hooks.zh.md)所规定的任务与延迟边界保持不变。省略 lefthook 的 production 或制品 `pnpm install` 现在会完成，而不再因 lefthook postinstall 导入失败。
