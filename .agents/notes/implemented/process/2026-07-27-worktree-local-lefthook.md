...
Hook installation is worktree-scoped. With `CI=true` or `GITHUB_ACTIONS=true`, the installer returns before Git discovery or mutation because automated jobs do not consume contributor hooks. With `LEFTHOOK=0`, or when the current worktree has no lefthook package or lefthook binary, it also returns without mutation so a production or artifact `pnpm install` that omitted lefthook can complete. Otherwise, it requires Git 2.26 or newer so `git config --show-scope` can report which scope supplied a value, upgrades a format-0 repository to format 1, enables `extensions.worktreeConfig`, and assigns the current worktree an absolute `core.hooksPath` at `$GIT_DIR/dsh-hooks`.
...
**Treat lefthook as a production dependency so artifact installs keep lefthook.** Rejected because lefthook is a contributor hook manager, not a runtime dependency. The installer must no-op when lefthook is absent.
