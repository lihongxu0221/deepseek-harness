...
export type HostToShell =
  ...
  | { type: 'listen'; host: string; port: number; url?: string }
  | { type: 'error'; text: string; zh?: string }
  | { type: 'focus'; pid?: number }
  | { type: 'quit' }

const SHELL_TO_HOST_TYPES = new Set(['show', 'start', 'stop', 'restart', 'settings', 'listen', 'window-missing', 'quit'])

/** Stopped (black) whale `.ico` filename, next to `dsh-web.exe` and under `apps/cli/assets/`. */
export const DSH_WEB_ICON_FILENAME = 'dsh-web.ico'

/** Running (DeepSeek 500 blue) whale `.ico` filename. */
export const DSH_WEB_RUNNING_ICON_FILENAME = 'dsh-web-running.ico'

/**
 * Locate a whale `.ico` for the splash and tray.
 * Packaged layouts keep files beside the exe; source / bundled CLI layouts keep
 * them at `apps/cli/assets/` relative to this module.
 * @param execPath - packaged executable path.
 * @param filename - `dsh-web.ico` or `dsh-web-running.ico`.
 * @returns the first existing candidate, or `undefined`.
 */
export function resolveDesktopIconPath(
  execPath: string,
  filename: string = DSH_WEB_ICON_FILENAME,
): string | undefined {
  const candidates = [
    join(dirname(execPath), filename),
    join(dirname(fileURLToPath(import.meta.url)), '..', 'assets', filename),
  ]
...
export interface WindowsDesktopShellIo {
   ...
   * @param env - environment including DSH_WEB_PARENT_PID, DSH_WEB_EXE, DSH_WEB_ICON, and DSH_WEB_ICON_RUNNING.
...
export function startWindowsDesktopShell(io: WindowsDesktopShellIo): WindowsDesktopShell {
  ...
  const runningIconPath = resolveDesktopIconPath(io.execPath, DSH_WEB_RUNNING_ICON_FILENAME)
  const child = io.spawn(
    ...
    ['-NoProfile', '-STA', '-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden', '-File', scriptPath],
    {
      ...
      ...(runningIconPath === undefined ? {} : { DSH_WEB_ICON_RUNNING: runningIconPath }),
    },
  )
