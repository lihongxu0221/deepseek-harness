...
describe('windows desktop shell protocol', () => {
  ...
  it('embeds the sibling PowerShell script byte-for-byte', () => {
    ...
    expect(source).toContain('function DisplayListenUrl')
    expect(source).toContain('$uri.GetLeftPart([UriPartial]::Authority)')
    expect(source).toContain('$url = DisplayListenUrl')
    expect(source).toContain('$script:ListenUrlOverride')
    expect(source).toContain('$msg.url')
    expect(source).toContain('$url = ListenUrl')
