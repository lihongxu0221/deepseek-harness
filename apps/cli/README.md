...
From the repository root, run `build-exe.bat` on Windows or `pnpm run build:web-exe`. The packer writes `VERSION` beside the launcher; a `winexeBuilder` or `winexeNew` push stamps `<package-version>.winexe.<GitHub run>` and publishes a prerelease zip.
