...
`scripts/build-web-exe.ts` deploys the same `web` profile into `dist-exe/dsh-web-<platform>-<arch>/` and wraps a thin `dsh-web` launcher that imports on-disk `lib/packaged-web-bin.js`, or an existing `.js`/`.cjs`/`.mjs` extra argument when a helper spawn()s the exe as Node. A first extra argument `plugin` runs on-disk `lib/bin.js` against `.config` instead of claiming the Windows single-instance GUI lock, so Plugin Market installs reach pnpm. The Web GUI is not a single-file pkg snapshot, because profile module fallback needs real packages. Double-clicking the launcher boots the GUI, opens the loopback URL in an Edge or Chrome app-mode window, and uses the executable directory as the invoking directory so Explorer's process cwd is not `System32`. Keep the whole folder together. A target machine does not need a system Node.js or Python install: the launcher embeds Node, and `node_modules/` ships beside it. On Windows the build marks the launcher as a GUI-subsystem executable so Explorer does not open a console. A splash window reports boot progress; the tray icon then owns lifetime. Closing the Chromium window hides the GUI only. Tray Show focuses a visible product window or reopens it; Settings opens `#settings`. Right-click the tray icon to show the main window, open the listen URL in the default browser, start or stop the web service, open Settings, or exit. On macOS/Linux, close the app window or the console to stop. The JSON-RPC `dsh-jsonrpc-agent-pkg` executable is a different product and still speaks stdio. The [Windows splash and tray Agent Note](../../../.agents/notes/implemented/feature/2026-08-19-windows-packaged-desktop-tray.md) owns that host.
