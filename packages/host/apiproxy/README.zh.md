...
`session.history` 会读取已附加 Session 的内存状态，或通过持久化检查冷日志，而不会恢复或发布 agent，然后按追加来源的消息边界分页：`maxMessages` 统计以追加方式进入 surface 的 `user/message` 和 `assistant/message` 事件，因此仅供模型使用的替换副本不占用配额。在此配额之外，每页还受工具事件预算约束——一旦页内已装入 80 个 `tool/call`／`tool/result` 事件，扫描只推进到下一个不拆分调用对的边界，因此一轮进行中的会话无法把数百次工具交换拉上同一页，任何 result 也绝不会脱离自己的 call 单独出现在页内。每一页仍是一段连续的原始事件区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。
