...
export abstract class AbstractApiClient implements IApiClient {
  ...
  /**
   * Mint a correlation id for an outbound client request.
   * Uses `crypto.getRandomValues`, which browsers expose on HTTP origins that
   * are not secure contexts (LAN IP literals). `crypto.randomUUID` is omitted there.
   * @returns a UUID v4 rpcId.
   */
