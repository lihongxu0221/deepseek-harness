...
常驻窗口的实时事件数超过 `WINDOW_EVENT_BUDGET`（800）时，Session 会从预算线上最早的 `turn/start` 边界处丢弃前缀——单轮独大时回退到 `step/start`，找不到合格边界则跳过淘汰——并重新打开 `hasMore`，让 loadOlder 继续负责被丢弃的区间。各 Definition 本就通过 windowGap 依赖容忍「从中途开始」的窗口；淘汰只是本地记账，绝不触发重取。
