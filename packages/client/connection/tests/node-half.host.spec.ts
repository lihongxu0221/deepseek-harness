...
describe('connection node half', () => {
  ...
  it('pins privileged methods to loopback even for a declared trusted authority', async () => {
    ...
    // Native desktop actions and preset authoring stay loopback-only.
    for (const method of [
...
describe('connection node half over a real HTTP server', () => {
  ...
  it('answers a declared LAN authority with 403 on native desktop and preset-authoring methods, over real HTTP', async () => {
    ...
    try {
      ...
      // Configuration reads and writes ride the outer trusted-host fence: a
      // same-origin LAN page already reaches session.prompt, so 404 here is
      // the empty proxy's carrier answer — the extra loopback pin did not fire.
      for (const method of [
        'settings.describe', 'settings.update', 'settings.replace', 'settings.mutate',
        'credentials.describe', 'credentials.set', 'credentials.unset',
