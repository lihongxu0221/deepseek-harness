...
- `ctx.workspaceRegistry.create(path, title?)` — canonicalizes `path` via `fs.realpath`, rejects a nonexistent or non-directory path, creates at most one record per PRIMARY directory, and prepends a new record to durable workspace order. Repeated calls for that primary return the existing workspace without changing its title; a path held only as an extra folder still creates a new workspace. Different paths may share a display title.
- `ctx.workspaceRegistry.get(id)` / `list()` / `resolveByPath(path)` — cache-served lookups. `list()` is synchronous and follows durable registry order; `resolveByPath` is async because it applies the same `realpath` canon and rejects a missing path rather than creating it. Resolution matches only the primary path.
- `Workspace.addFolder(path)` / `removeFolder(path)` / `setPrimaryFolder(path)` — adopt or drop an extra directory, or promote an owned extra to primary (new-session cwd). `addFolder` canonicalizes like create, rejects a missing or non-directory path, and no-ops when this workspace already owns the path; a path another workspace owns — as extra or primary — is accepted. `removeFolder` rejects the primary path and no-ops an unknown extra folder; it never deletes the directory. `setPrimaryFolder` swaps `path` with the named extra, no-ops the current primary, rejects an unknown path, and rejects a path that is already another workspace's primary. Extra folders expand session membership and workspace-write roots; new sessions and the `AGENTS.md` walk use the current primary as cwd. Existing session header cwds stay unchanged.
...
- `Workspace.attachSession(id)` — validates a live or persisted session header cwd against the primary path or any extra folder and prepends a new id. Unknown sessions, absent/unresolvable/non-directory cwd values, mismatches, and sessions another workspace already accounts reject without writing. `detachSession` removes only the candidate index entry.
...
- `Workspace.sessionIds` — synchronous id-plus-canonical-cwd membership projection in durable candidate order. Missing headers, invalid cwd values, and mismatches are filtered; the next workspace mutation prunes them. A medium indexing one session under two workspaces, claiming one primary path from two records, claiming one path twice in the same record, or diverging from durable workspace order rejects at startup.
...
- Extra folders do not expand the `AGENTS.md` instruction walk, `@`-mention search, or `fs-search` default root; those stay on the session cwd ([decision](../../../.agents/notes/implemented/feature/2026-08-24-workspace-shared-extra-folders.md)).
