...
- `ctx.workspaceRegistry.create(path, title?)` — canonicalizes `path` via `fs.realpath`, rejects a nonexistent or non-directory path, creates at most one record per canonical path (primary or extra folder), and prepends a new record to durable workspace order. Repeated calls for that path return the existing workspace without changing its title; different paths may share a display title.
- `ctx.workspaceRegistry.get(id)` / `list()` / `resolveByPath(path)` — cache-served lookups. `list()` is synchronous and follows durable registry order; `resolveByPath` is async because it applies the same `realpath` canon and rejects a missing path rather than creating it. Resolution matches the primary path or any extra folder.
- `Workspace.addFolder(path)` / `removeFolder(path)` / `setPrimaryFolder(path)` — adopt or drop an extra directory, or promote an owned extra to primary (new-session cwd). `addFolder` canonicalizes like create, rejects a missing or non-directory path, no-ops when this workspace already owns the path, and rejects a path owned by another workspace. `removeFolder` rejects the primary path and no-ops an unknown extra folder; it never deletes the directory. `setPrimaryFolder` swaps `path` with the named extra, no-ops the current primary, and rejects an unknown path. Extra folders expand session membership and workspace-write roots; new sessions and the `AGENTS.md` walk use the current primary as cwd. Existing session header cwds stay unchanged.
...
- `Workspace.attachSession(id)` — validates a live or persisted session header cwd against the primary path or any extra folder and prepends a new id. Unknown sessions, absent/unresolvable/non-directory cwd values, and mismatches reject without writing. `detachSession` removes only the candidate index entry.
