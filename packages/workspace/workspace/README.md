...
- `Workspace.addFolder(path)` / `removeFolder(path)` — adopt or drop an extra directory. `addFolder` canonicalizes like create, rejects a missing or non-directory path, no-ops when this workspace already owns the path, and rejects a path owned by another workspace. `removeFolder` rejects the primary path and no-ops an unknown extra folder; it never deletes the directory. Extra folders expand session membership and workspace-write roots; new sessions still use the primary path as cwd.
