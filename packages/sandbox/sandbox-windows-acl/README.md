...
- **ACL grants are standing directory mutations.** They persist if the process dies mid-run; the primary workspace ACE is standing BY DESIGN (never revoked — the reuse cache), extra-folder ACEs are revoked when that folder leaves the workspace, and temp ACEs are revoked by `dispose()` (`init()` also revokes an already-applied temp grant when a later step fails). The POC's documented manual cleanup (`icacls <dir> /remove '*S-1-4-…'`) fails on this platform with `ERROR_NONE_MAPPED` (1332) — revoke through this module instead. An unclean shutdown needs no self-healing for the workspace ACE: the derived SID re-hits the standing ACE on the next provision (skipping the apply); the write-SID ACE never accumulates a second identity per restart because the identity IS the workspace.
